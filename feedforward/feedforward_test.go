...
// Tests for NoFeedForward
func TestNoFeedForwardCalculate(t *testing.T) {
	tests := []struct {
		name         string
		setpoint     float64
		velocity     float64
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := noff.Calculate(tt.setpoint, tt.velocity, tt.acceleration)
			if result != tt.expected {
				t.Errorf("Calculate() = %f, expected %f", result, tt.expected)
			}
		})
	}
}

// Benchmark tests for NoFeedForward

func BenchmarkNoFeedForwardCalculate(b *testing.B) {
	noff := &NoFeedForward{}
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		noff.Calculate(math.Pi/4, 2.0, 1.5)
	}
}
